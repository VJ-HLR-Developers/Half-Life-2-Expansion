	...
	if IsValid(data.HitEntity) && data.HitEntity != Entity(0) then
		...
		timer.Simple(SoundDuration(snd) *1.5,function()
			if IsValid(fakeEnt) then
				VJ_EmitSound(fakeEnt,"vj_hlr/hl2_npc/ministrider/flechette_explode" .. math.random(1,3) .. ".wav",95)
				ParticleEffect("hunter_projectile_explosion_1",data.HitPos,Angle(0,0,0),nil)
				util.VJ_SphereDamage(IsValid(self) && self or IsValid(owner) && owner or fakeEnt, IsValid(owner) && owner or IsValid(self) && self or fakeEnt, data.HitPos, 128, 12, bit.bor(DMG_BLAST,DMG_DISSOLVE), true, true)
